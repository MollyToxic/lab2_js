...
function drawMarkers(markers){
    for(let marker of markers){
        context.fillStyle = marker.selected ? 'black': 'green';
        // marker.x = 3;
        // marker.y = 3;
...
function drawRect(point1, point2, marker1, marker2){
    context.fillStyle = 'rgba(207, 0, 203, 1)';
    ...
    if(res !== -1){
        ...
        if(innerLine){
            const innerColor = 'rgba(0, 207, 69, 1)';
...
function updateMarkers(markers){
    const cellColor = 'rgba(207, 203, 0, 1)';
    if(fullLine){
...
function setup(){
    ...
    // Указание координат маркеров должно быть здесь!
    //markers[0].x = 1;
    //markers[0].y = 1;
    // getCoordinates();
...
function getCoordinates() {
    markers[0].x = Number(document.getElementById("firstMarkerX").value);
    markers[0].y = Number(document.getElementById("firstMarkerY").value);
    if (markers[0].x > 1)
    {
        markers[0].x--;
    }
    ...
    markers[1].x = Number(document.getElementById("secondMarkerX").value);
    markers[1].y = Number(document.getElementById("secondMarkerY").value);
    if (markers[1].x > 1)
    {
        markers[1].x--;
    }
    ...
    if (markers[1].x == 0 && markers[1].y == 0)
    {
        markers[1].x = 29;
        markers[1].y = 29;
    }
    ...
    markers[2].x = Number(document.getElementById("thirdMarkerX").value);
    markers[2].y = Number(document.getElementById("thirdMarkerY").value);
    if (markers[2].x > 1)
    {
        markers[2].x--;
    }
    ...
    if (markers[2].x == 0 && markers[2].y == 0 )
    {
        markers[2].x = 29;
        markers[2].y = 0;
    }

    markers[3].x = Number(document.getElementById("fourthMarkerX").value);
    markers[3].y = Number(document.getElementById("fourthMarkerY").value);
    if (markers[3].x > 1)
    {
        markers[3].x--;
    }
    ...
    if (markers[3].x == 0 && markers[3].y == 0 )
    {
        markers[3].x = 0;
        markers[3].y = 29;
    }
